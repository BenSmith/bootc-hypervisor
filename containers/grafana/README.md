...
2. **Enable once to create volume directories (will fail — that's expected):**
   ```bash
   sudo workload-ctl enable grafana
   ```

3. **Copy the config template and edit:**
   ```bash
   sudo cp /usr/share/workload-containers/grafana/grafana.ini \
...
```bash
echo -n "your-password" | sudo workload-ctl secret create grafana-admin-password
```
...
```bash
workload-ctl logs grafana
workload-ctl status grafana
```
