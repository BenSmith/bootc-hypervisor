...
2. **Copy your WireGuard config and enable:**
   ...
   sudo workload-ctl enable pihole-vpn
   ```
...
4. **Download blocklists:**
   ```bash
   sudo workload-ctl exec pihole-vpn pihole -g
   ```
...
```bash
echo -n "your-password" | sudo workload-ctl secret create pihole-vpn-webpassword
```
...
```bash
workload-ctl logs pihole-vpn
workload-ctl status pihole-vpn

# Check public IP used for DNS (should show VPN server)
sudo workload-ctl exec pihole-vpn curl https://ipinfo.io/ip
```
