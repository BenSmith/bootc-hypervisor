...
# three pieces below are yours to write. They are commented out because this
...
#
# TWO ROUTES, BOTH NEEDED. ca_certs installs it into the guest's system trust
# store, which covers almost everything; the write_files copy at a fixed path is
# what the five environment variables name, for the runtimes that carry their
# own root list and never consult the system store. Either alone leaves a
# measured population of clients failing.
#
# write_files (append to the list above):
...
#
# ca_certs (top level; ${WORKLOADCTL_VM_EGRESS_CA} is the raw multi-line PEM,
# which needs the six-space indent of the block scalar it is spliced into):
#   ca_certs:
#     trusted:
#       - |
