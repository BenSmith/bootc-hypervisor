...
class TestBuildCloudInitIsoTemplateMode(unittest.TestCase):
    ...
    def test_a_commented_context_does_not_satisfy_the_check(self):
        ...
        carries this exact mount as a COMMENTED example, so a seed copied from
        the reference and only half finished — mount uncommented, context line
        left behind in the comment block — would otherwise pass.
        """
        self._seed(
            "# home-fedora /home/fedora virtiofs "
            'defaults,context="system_u:object_r:user_home_t:s0" 0 0\n'
            + self._HOME_MOUNT
        )
        with self.assertRaises(RuntimeError) as ctx:
            self._run_build(self._home_cfg())
