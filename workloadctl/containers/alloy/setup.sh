#!/bin/bash
# Host setup for the alloy workload.
#
# Usage:
...
MODULE_NAME="alloy"
WORK_DIR=""
# return 0 explicitly: under `set -e`, a cleanup that ends on a false test
# (empty WORK_DIR, e.g. the disable path) would otherwise make the EXIT trap
# propagate exit status 1 as the script's status.
