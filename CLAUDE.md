...
- **Reading workload container logs:** units run `--log-driver=passthrough`, so `podman logs` fails (`cannot read logs: this container is not logging output`) and `workloadctl logs` (which shells to `podman logs`) comes up empty. `journalctl -u workload-<name>-<container>.service` shows only systemd's own start/stop lines too — the rootless container reparents into `user@<uid>.service`/`libpod-<pod>.scope`, not the `podman run` supervisor unit. The app output *is* in the journal, stamped with the container name as syslog identifier: read it with `journalctl -t workload-<name>-<container>` (add `-f` to tail).
